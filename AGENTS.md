...
- `src/components/layout/Header.tsx` — Visual refinements, now renders NotificationBell
- `src/components/shared/NotificationBell.tsx` — Visual refinements
- `src/components/layout/AppLayout.tsx` — Imports Header, mobile pt-16 fix
...
- `src/app\(dashboard)\loans\page.tsx` — Currency fix
- `src/app\(dashboard)\subscriptions\page.tsx` — Currency fix
- `src/app\(dashboard)\recurring\page.tsx` — Currency fix, always-visible buttons

### Next Steps
- Test all interactive elements (filter chips, upcoming item clicks, FAB on mobile, notification dropdown).
- Apply visual improvements to other pages (expenses, income, budgets, etc.) if desired.
