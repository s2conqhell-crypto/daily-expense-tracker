...
import {
  MobilePage, MobilePageHeader, MobileSection,
  MobileEmptyState,
            ...
            {loading ? (
              <div className="px-5 pb-5">
                <div className="grid grid-cols-7 gap-1">
                  {Array.from({ length: 35 }).map((_, i) => <div key={i} className="aspect-square rounded-xl bg-white/5 animate-pulse" />)}
                </div>
              </div>
            ) : (
              <div className="px-5 pb-5">
          ...
          <div className="bg-[#161a27] rounded-[20px] border border-white/[0.06] p-5">
            {selectedDate ? (
              ...
              <MobileEmptyState
                icon={<Receipt className="h-8 w-8" />}
                title="Select a date"
                description="Click a date to view transactions"
              />
            )}
          </div>
        </MobileSection>
